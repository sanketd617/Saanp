class Saanp {
    constructor(initialSize, ctx, cells) {
        this.ctx = ctx;
        this.cells = cells;
        this.body = [{x: initialSize + 1, y: 1}];
        this.cellSize = this.ctx.canvas.height / this.cells;
    ...
    update() {
        ...
        if (head.x + this.direction.x < 0 || head.x + this.direction.x >= this.cells
            || head.y + this.direction.y < 0 || head.y + this.direction.y >= this.cells) {
            this.dead = true;
            return;
        }
